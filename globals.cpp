...
void MicroRoughnessRotateVector(long double v[3], const long double n[3], long double theta, long double phi) //rotates vector around the normal vector, n, by theta and phi using the Rodrigues rotation formula
//phi rotated counterclockwise to normal vector
//theta rotated away from normal
{
//phi rotation
//==============
long double vParallel[3]; //the component of v parallel to n
long double vPerpendicular[3]; //the component of v perpendicular to n
long double n_vCrossProduct[3]; //this is a vector that is perpendicular to both v and n 
long double vRotPhi[3]; //rotated vector of v by phi

long double v_nDotProduct = v[0]*n[0]+v[1]*n[1]+v[2]*n[2]; //dot product of v and n
//the following is the cross product of v and n
n_vCrossProduct[0]=n[1]*v[2]-n[2]*v[1];
n_vCrossProduct[1]=n[2]*v[0]-n[0]*v[2];
n_vCrossProduct[2]=n[0]*v[1]-n[1]*v[0];

for (int i = 0; i < 3; i++) { 
	vParallel[i]=v_nDotProduct*n[i];
	vPerpendicular[i]=v[i]-vParallel[i];
	vRotPhi[i]=vPerpendicular[i]*cos(phi)+n_vCrossProduct[i]*sin(phi)+vParallel[i]; //Vector rotated by phi using Rodrigues formula
}

//theta rotation
//============
//Once the vector has been rotated around the normal (phi direction), it is rotated in the theta direction
//This is done by rotating it around wNormal, which is defined below 

long double w[3]; //the vector perpendicular to vRotPhi and n
long double wNormal[3]; //the normalized w vector, around which vRotPhi is rotated
w[0]=n[1]*vRotPhi[2]-n[2]*vRotPhi[1];
w[1]=n[2]*vRotPhi[0]-n[0]*vRotPhi[2];
w[2]=n[0]*vRotPhi[1]-n[1]*vRotPhi[0];
long double wMagnitude = sqrt(w[0]*w[0] + w[1]*w[1] + w[2]*w[2]);
for (int i = 0; i < 3; i++) {
	wNormal[i]=w[i]/wMagnitude;
}

long double vRotPhiParallel[3]; //the component of v parallel to w
long double vRotPhiPerpendicular[3]; //the component of v perpendicular to w
long double wNormal_vRotPhiCrossProduct[3]; //this is a vector that is perpendicular to both v and w 
long double vRotPhiTheta[3]; //rotated vector of v by phi, and then theta

long double vRotPhi_wNormalDotProduct = vRotPhi[0]*wNormal[0]+vRotPhi[1]*wNormal[1]+vRotPhi[2]*wNormal[2]; //dot product of vRotPhi and wNormal

//the following is the cross product of vRotPhi and wNormal
wNormal_vRotPhiCrossProduct[0]=wNormal[1]*vRotPhi[2]-wNormal[2]*vRotPhi[1];
wNormal_vRotPhiCrossProduct[1]=wNormal[2]*vRotPhi[0]-wNormal[0]*vRotPhi[2];
wNormal_vRotPhiCrossProduct[2]=wNormal[0]*vRotPhi[1]-wNormal[1]*vRotPhi[0];

for (int i = 0; i < 3; i++) {
	vRotPhiParallel[i]=vRotPhi_wNormalDotProduct*wNormal[i];
	vRotPhiPerpendicular[i]=vRotPhi[i]-vRotPhiParallel[i];
	vRotPhiTheta[i]=vRotPhiPerpendicular[i]*cos(theta)+wNormal_vRotPhiCrossProduct[i]*sin(theta)+vRotPhiParallel[i]; //Vector rotated by theta using Rodrigues formula
}
