...
//		printf("Diffuse transmission! Erefl=%LG neV w_e=%LG w_s=%LG\n",Enormal*1e9,phi_r/conv,theta_r/conv);
	...
	void DoMicroRoughnessReflection(long double FermiDiff, long double x1, long double y1[6], long double &x2, long double y2[6], const long double normal[3], solid *entering, MRHelperPointerless MRFunctions) {
		bool found = false;
		...
		while (found == false) {
			...
			double AngCut = entering->mat.AngCut;			
			long double prob_actual = MRFunctions.ProbIplus(E, FermiDiff, theta_i, theta_o_test, phi_o_test, b, w, AngCut);
	...
	void DoMicroRoughnessTransmission(long double FermiDiff, long double x1, long double y1[6], long double &x2, long double y2[6], const long double normal[3], solid *entering, MRHelperPointerless MRFunctions) {
		bool found = false;
		...
		while (found == false) {
			...
			double AngCut = entering->mat.AngCut;
			long double prob_actual = MRFunctions.ProbIminus(E, FermiDiff, theta_i, theta_o_test, phi_o_test, b, w, AngCut);
			if (prob_test <= prob_actual) {
				x2 = x1;
				for (int i = 0; i < 3; i++)
				y2[i] = y1[i];
