				...
				switch (Interaction) { //This switch changes to the appropriate model depending on what the int Interaction is
					...
					case 2: { //Specular-MicroRoughness case
						...
						if (valid) { //Do MR transmission only if the conditions are valid
							...
							long double MRprob = entering->mat.GetMRIntTransProbability(theta_i, E);
							//cout << '\n' <<'\n' << " MicroRoughness Prob = " << diffuseprob << '\n' << '\n' << '\n';
					
							if (prob1<MRprob) { //this is for MRM transmission	
								//************** MicroRoughness Transmission **************
								DoMicroRoughnessTransmission(Estep, x1, y1, x2, y2, normal, entering);
								traversed = true;
		...
		if (refl){
			switch (Interaction) { //This switch changes to the appropriate model depending on what value the int Interaction takes
				...
				case 2: { //Specular-MicroRoughness case
					...
					//cout << "MR Transmission conditions are valid!" << endl;
						long double prob1 = mc->UniformDist(0,1); //random dice to determine if it is MRM or Snell's Law
						long double MRprob = entering->mat.GetMRIntProbability(theta_i, E);
						//cout << '\n' <<'\n' << " MicroRoughness Prob = " << diffuseprob << '\n' << '\n' << '\n';
				
						if (prob1<MRprob) { //this is for MRM reflection
							//************** MicroRoughness Reflection **************	
							DoMicroRoughnessReflection(Estep, x1, y1, x2, y2, normal, entering);
						}				
...
//		printf("Diffuse transmission! Erefl=%LG neV w_e=%LG w_s=%LG\n",Enormal*1e9,phi_r/conv,theta_r/conv);
	...
	void DoMicroRoughnessReflection(long double Estep, long double x1, long double y1[6], long double &x2, long double y2[6], const long double normal[3], solid *entering) {
		bool found = false;
		long double E = Ekin(&y1[3]); //Energy of Neutron
		...
		while (found == false) {
			...
			long double MRMaxProb = entering->mat.GetMRMaxProbability(theta_i, E);
			long double prob_test = mc->UniformDist(0,1.5*MRMaxProb); //this is the probability value for the given phi and theta value above, 
								     //which will be compared to see if it is above or below the actual value computed
			long double prob_actual = entering->mat.GetMRProbability(theta_i, E, Estep, theta_o_test, phi_o_test);
	...
	void DoMicroRoughnessTransmission(long double Estep, long double x1, long double y1[6], long double &x2, long double y2[6], const long double normal[3], solid *entering) {
		bool found = false;
		...
		while (found == false) {
			long double phi_o_test = mc->UniformDist(-1*pi,pi); //this is the potential outgoing phi value for the transmitted neutron
			long double theta_o_test = mc->UniformDist(0,0.5*pi); //this is the potential outgoing theta value for the transmitted neutron
			long double MRMaxTransProb = entering->mat.GetMRMaxTransProbability(theta_i, E);
			long double prob_test = mc->UniformDist(0,1.5*MRMaxTransProb); //this is the probability value for the given phi and theta value above, 
								     //which will be compared to see if it is above or below the actual value computed
			long double prob_actual = entering->mat.GetMRTransProbability(theta_i, E, Estep, theta_o_test, phi_o_test);
			if (prob_test <= prob_actual) {
				x2 = x1;
				for (int i = 0; i < 3; i++)
				y2[i] = y1[i];
